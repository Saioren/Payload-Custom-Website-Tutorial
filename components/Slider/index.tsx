...
export const SliderProvider: React.FC = ({ children }) => {
  ...
  const currentSlide = slides.findIndex(
    (slide) => slide && slide.isIntersecting
  );
  ...
  const next = useCallback(() => {
    if (containerRef.current) {
      ...
      const scrollPos = passedSlides.reduce(
        (pos, slide) => pos + slide.width,
        0
      );
...
const Slider: React.FC<Props> = ({ children, hideScrollbar = false }) => {
  ...
  return (
    <div
      className={joinClassNames([
        classes.wrap,
        enableDragHandle && classes.grabCursor,
      ])}
      style={
        hideScrollbar
          ? {
